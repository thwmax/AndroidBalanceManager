...
interface MoneyTransferDao {
    ...
    fun getTransfersToAccount(moneyAccount: Int): List<MoneyTransaction>

    @Query("SELECT * from money_transfer where origin_account = :moneyAccount")
    fun getTransfersFromAccount(moneyAccount: Int): List<MoneyTransaction>

    @Query("SELECT * from money_transfer where operation_date >= :dateStart and operation_date <= :dateEnd")
    fun getTransfersBetweenDates(dateStart: Date, dateEnd: Date): List<MoneyTransaction>
}